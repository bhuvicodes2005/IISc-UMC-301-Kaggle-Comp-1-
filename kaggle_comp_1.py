...
# ================= CONFIG =================
FAST_RUN = False  # Set True for quick tests
N_FOLDS = 3 if FAST_RUN else 5
RANDOM_STATE = 42
...
# ================= IMPUTE + SCALE for models that need it =================
imputer = SimpleImputer(strategy="median")
scaler = StandardScaler()
...
# ================= MODELS =================
base_models = {
    "lgb": lgb.LGBMClassifier(n_estimators=800, random_state=RANDOM_STATE, verbose=-1),
    "cat": cb.CatBoostClassifier(iterations=800, random_seed=RANDOM_STATE, verbose=0),
    "xgb": xgb.XGBClassifier(n_estimators=800, eval_metric="auc", use_label_encoder=False, random_state=RANDOM_STATE),
    "lr": LogisticRegression(max_iter=500, random_state=RANDOM_STATE),
    "ada": AdaBoostClassifier(n_estimators=300, random_state=RANDOM_STATE),
    "etc": ExtraTreesClassifier(n_estimators=500, random_state=RANDOM_STATE, n_jobs=-1)
}
...
for fold, (train_idx, valid_idx) in enumerate(skf.split(X, y)):
    ...
    for m_idx, (name, model) in enumerate(base_models.items()):
        ...
        if name in ["lr", "ada", "etc"]:  # NEED imputed+scaled
            model.fit(X_tr_scaled, y_tr)
            oof_preds[valid_idx, m_idx] = model.predict_proba(X_val_scaled)[:, 1]
            test_preds[:, m_idx] += model.predict_proba(X_test_scaled)[:, 1] / N_FOLDS
        else:  # Tree-based → raw data with NaNs allowed
            model.fit(X_tr, y_tr)
            oof_preds[valid_idx, m_idx] = model.predict_proba(X_val)[:, 1]
            test_preds[:, m_idx] += model.predict_proba(X_test)[:, 1] / N_FOLDS

    fold_auc = roc_auc_score(y_val, oof_preds[valid_idx].mean(axis=1))
    print(f"Fold {fold+1} mean-base AUC: {fold_auc:.4f}")
